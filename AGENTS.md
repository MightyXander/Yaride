...
- **Schema version:** SQLite has a `schema_version(id, version)` table. `app.db.SCHEMA_VERSION` is the current code version. Linear migrations by version are introduced in Этап 1 (see `docs/superpowers/specs/2026-05-11-etap-1-data-resilience.md`). Этап 2 adds performance indexes (`app/db.py`, migration to v3); see README «Индексы и запросы». Этап 4 adds `chat_anchors` (v4) and reply-aux column + drops `bot_chat_messages` (v5). Админка adds `users.is_banned`, `admin_users`, `admin_audit_log` (v6). v7 removes non-Yaroslavl localities (Рыбинск, Тутаев etc.) and related trips/bookings. v8 adds Mini App fields: `users.car_model/car_color/car_plate` and `trips.comment`. v9 adds `trip_templates` (маршруты-шаблоны водителя: маршрут + дефолты цены/мест/комментария; `schedule_*` nullable — задел под авто-публикацию).
- **Telegram Mini App:** `miniapp/` is the frontend (Vite + React + TS + Tailwind, design tokens from Stitch `DESIGN.md`, native Telegram WebApp integration in `src/telegram/`). `webapp_api/` is the FastAPI backend over the same `yaride.db` via `app.repo`/services, authenticated by Telegram `initData` (HMAC with `BOT_TOKEN`). Run API: `py -3 -m webapp_api` (port 8080). Run frontend: `cd miniapp && npm run dev` (proxies `/api` → 8080). Set `WEBAPP_DEV_USER_ID` to develop the frontend in a plain browser without Telegram.
