...
- **Schema version:** SQLite has a `schema_version(id, version)` table. `app.db.SCHEMA_VERSION` is the current code version. Linear migrations by version are introduced in Этап 1 (see `docs/superpowers/specs/2026-05-11-etap-1-data-resilience.md`). Этап 2 adds performance indexes (`app/db.py`, migration to v3); see README «Индексы и запросы». Этап 4 adds `chat_anchors` (v4) and reply-aux column + drops `bot_chat_messages` (v5). Админка adds `users.is_banned`, `admin_users`, `admin_audit_log` (v6). v7 removes non-Yaroslavl localities (Рыбинск, Тутаев etc.) and related trips/bookings. v8 adds Mini App fields: `users.car_model/car_color/car_plate` and `trips.comment`. v9 adds `trip_templates` (маршруты-шаблоны водителя: маршрут + дефолты цены/мест/комментария; `schedule_*` nullable — задел под авто-публикацию). v10 adds `users.driver_moderation_status` (`pending`/`approved`/`rejected`): новые водители ждут одобрения в админке (`/drivers/pending`) до доступа к созданию поездок. v11 сбрасывает `users.min_passenger_rating` в NULL — фильтр по рейтингу пассажиров по умолчанию выключен.
- **Telegram Mini App:** `miniapp/` — TanStack Start + React 19 + Vite 7 + Tailwind v4, file routes in `src/routes/`, API client `src/lib/api.ts` (auth: `X-Init-Data`), TanStack Query in `src/lib/queries.ts`. UI-only state in `src/lib/store.tsx`. `webapp_api/` — FastAPI over `yaride.db`. Run API: `py -3 -m webapp_api` (8080). Run frontend: `cd miniapp && npm run dev` (прокси `/api` → 8080). `WEBAPP_DEV_USER_ID` — dev в браузере. Новые эндпоинты: `GET /api/history`, `GET /api/ratings/pending`. См. `miniapp/README.md`.
- **Yaroslavl-only navigation:** The trip flow now shows Yaroslavl districts directly (no city selection step). Seeds, geo-stops and coordinate overrides contain only Yaroslavl data. Migration v7 cleans up any legacy points/trips outside Yaroslavl.
